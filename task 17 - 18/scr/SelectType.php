...
class SelectType
{
    ...
    public function getWrite(): WriteInterface
    {
        switch ($this->type) {
            case 'xml' :
                return new WriteXml($this->pathFile);
            case 'csv' :
                return new WriteCsv($this->pathFile);
            case 'json' :
                return new WriteJson($this->pathFile);
        }
        return null;
    }
    ...
    public function getRead(): ?ReadInterface
    {
        switch ($this->type) {
            case 'xml' :
                return new ReadXml($this->pathFile);
            case 'csv' :
                return new ReadCsv($this->pathFile);
            case 'json' :
                return new ReadJson($this->pathFile);
        }
        return null;
    }
}